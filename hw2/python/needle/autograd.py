    ...
    for node in reverse_topo_order:
        ...
        if len(node.inputs) > 0:
            ...
            if isinstance(gradient, tuple):
                for i, pre_node in enumerate(node.inputs):
                    node_to_output_grads_list.setdefault(pre_node, [])
                    node_to_output_grads_list[pre_node].append(gradient[i])
            else:
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    for pre_node in node.inputs:
        if not visited.get(pre_node, False):
